...
export default async function handler(req,res){
  ...
  const c=cookies(req);
  const account=c['__Host-brain_google_account'];
  const oauthState=c['__Host-brain_google_state'];
  const verifier=c['__Host-brain_google_verifier'];
  const {code,state}=req.query||{};
  if(!code||!state||!account||!oauthState||state!==oauthState||!verifier) return res.status(400).send('OAuth non valido.');
  ...
  try{
    const body=new URLSearchParams({client_id:clientId,client_secret:clientSecret,code:String(code),code_verifier:verifier,grant_type:'authorization_code',redirect_uri:`${base}/api/google-connect-callback`});
    ...
    if(!token.refresh_token) return res.status(400).send('Google non ha restituito un refresh token. Revoca il consenso dell’app e riprova.');
    ...
    res.setHeader('Set-Cookie',[clear('__Host-brain_google_state'),clear('__Host-brain_google_verifier'),clear('__Host-brain_google_account')]);
    return res.redirect(302,'/brain/index.html?google=connected');
  }catch(error){
    console.error('google-connect-callback',error);
    return res.status(503).send('Integrazione Google temporaneamente non disponibile.');
  }
}
