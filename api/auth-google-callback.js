...
import { recordAuthEvent } from '../lib/brain-security.js';
...
export default async function handler(req,res){
  ...
  const c=cookies(req);
  const stateCookie=c['__Host-brain_oauth_state'];
  const verifier=c['__Host-brain_oauth_verifier'];
  if(!code||!state||!stateCookie||state!==stateCookie||!verifier) {
    await recordAuthEvent(req,'google_login_invalid','Invalid Google OAuth callback','warning');
    return res.status(400).send('OAuth non valido.');
  }
  ...
  if(!tokenRes.ok) {
    await recordAuthEvent(req,'google_login_failed','Google token exchange failed','warning');
    return res.status(401).send('Autenticazione Google fallita.');
  }
  ...
  if(!user.email_verified||email!==allowed) {
    await recordAuthEvent(req,'google_login_denied','Unauthorized Google account','warning');
    return res.status(403).send('Account non autorizzato.');
  }
  try{
    const sessionCookie=await createDeviceSession(req,{authMethod:'google',userEmail:email});
    await recordAuthEvent(req,'google_login_success','Google login succeeded','info');
    res.setHeader('Set-Cookie',[sessionCookie,clear('__Host-brain_oauth_state'),clear('__Host-brain_oauth_verifier')]);
    return res.redirect(302,'/brain/');
  }catch(error){
    console.error('google session create',error?.message||error);
